    ...
    try {
      ...
      const processImage = async () => {
        try {
          ...
          // Step 2: Extract text using Claude
          const { text, error: textError } =
            await extractTextWithClaude(processedImage);
          if (textError) throw textError;

          // Step 3: Detect diagrams using YOLOv8
          const { diagrams, error: diagramError } =
            await detectDiagramsWithYOLO(processedImage);
          if (diagramError) throw diagramError;

          // Combine results
          const results = { text, diagrams };
